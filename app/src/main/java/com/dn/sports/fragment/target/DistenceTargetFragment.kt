...
class DistenceTargetFragment : BaseFragment() {
...
//    var targetDistanceNum = StepUserManager.getInstance().getTargetDistanceNum(requireContext())
    ...
    override fun initViewAction(view: View?) {
        targetStepNum = StepUserManager.getInstance().getTargetDistanceNum(requireContext()).toInt()
        val list = arrayListOf<Int>()
        for (i in minStep..maxStep step 100) {
            list.add(i)
        }
