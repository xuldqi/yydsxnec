...
public class OpenFileUtils {
     ...
     * 使用自定义方法打开文件
     */
    public static void openFile(Context activityFrom, File file) {
        Intent intent = new Intent();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            //  此处注意替换包名，
            Uri contentUri = FileProvider.getUriForFile(activityFrom, "com.dn.sports.fileprovider", file);
            EyeLog.logi("openFile uri   " + contentUri.getPath());
...
//            intent.setDataAndType(contentUri, "image/*");
    ...
    public static void openFile(Context activityFrom, Uri uri,String type) {
        Intent intent = new Intent();
